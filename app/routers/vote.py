...
def vote(vote:schemas.Vote,db:Session = Depends(database.get_db),current_user:int = Depends(oauth2.get_current_user)):
    ...
    if not post:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,detail=f"post with id  {vote.id} does not exist")
    ...
    if(vote.dir==1):
        if found_vote:
            raise HTTPException(status_code=status.HTTP_409_CONFLICT,detail="user "
                                 f"{current_user.id} has already voted on post {vote.post_id}")
